...
class Stem(
    ...
    fun CalculateEnd(): WhiteNote {
        return if (direction == Up) {
            ...
            throw NullPointerException() /* Shouldn't happen */
        }
    }
    ...
    private fun DrawCurvyStem(canvas: Canvas, paint: Paint, ytop: Int, topstaff: WhiteNote) {
        ...
        val xstart = if (side == LeftSide) SheetMusic.LineSpace / 4 + 1 else SheetMusic.LineSpace / 4 + SheetMusic.NoteWidth
        if (direction == Up) {
