...
class ColorDialog(context: Context, private val listener: ColorChangedListener, private val selectedColor: Int) : Dialog(context) {
    override fun onCreate(state: Bundle?) {
        ...
        val listen = object : ColorChangedListener {
...
internal class ColorView(context: Context, private val listener: ColorChangedListener, /* Currently selected color */private var selectedColor: Int) : View(context) {
    ...
    private fun initColorRings() {
        ...
        for (i in 0..63) {
            ...
            val s: Shader = SweepGradient(0f, 0f, colorsForRing(i / 64.0f), null)
     ...
     */
    override fun onDraw(canvas: Canvas) { // TODO: Avoid object allocations during draw to enhance performance
        colorPreview?.let { canvas.drawRoundRect(RectF((center / 10).toFloat(), (center / 10).toFloat(), (center / 4).toFloat(), (center / 4).toFloat()), 5f, 5f, it) }
        ...
        for (i in 1 until colorRings.size) {
            ...
            colorRings[i]?.let { canvas.drawOval(RectF(-radius, -radius, radius, radius), it) }
        }
    }
