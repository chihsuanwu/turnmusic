...
class Staff(symbols: ArrayList<MusicSymbol>, key: KeySignature,
    ...
    private val keys: Array<AccidentalSymbol?>
    /** The key signature symbols  */
    private val showMeasures: Boolean
    ...
    /** Draw the lyrics  */
    private fun DrawLyrics(canvas: Canvas, paint: Paint) { /* Skip the left side Clef symbol and key signature */
    ...
    private fun DrawMeasureNumbers(canvas: Canvas, paint: Paint) { /* Skip the left side Clef symbol and key signature */
    ...
    fun ShadeNotes(canvas: Canvas, paint: Paint, shade: Int,
                   currentPulseTime: Int, prevPulseTime: Int, x_shade_arg: Int): Int { /* If there's nothing to unshade, or shade, return */
        var x_shade = x_shade_arg
        ...
        for (i in symbols.indices) {
            ...
            val end = if (i + 2 < symbols.size && symbols[i + 1] is BarSymbol) {
