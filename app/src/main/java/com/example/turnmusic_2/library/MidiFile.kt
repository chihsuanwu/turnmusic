...
class MidiFile(rawdata: ByteArray?,
    ...
    private fun EventName(ev: Int): String {
        return if (ev >= EventNoteOff && ev < EventNoteOff + 16) "NoteOff" else if (ev >= EventNoteOn && ev < EventNoteOn + 16) "NoteOn" else if (ev >= EventKeyPressure && ev < EventKeyPressure + 16) "KeyPressure" else if (ev >= EventControlChange && ev < EventControlChange + 16) "ControlChange" else if (ev >= EventProgramChange && ev < EventProgramChange + 16) "ProgramChange" else if (ev >= EventChannelPressure && ev < EventChannelPressure + 16) "ChannelPressure" else if (ev >= EventPitchBend && ev < EventPitchBend + 16) "PitchBend" else if (ev == MetaEvent.toInt()) "MetaEvent" else if (ev == SysexEvent1.toInt() || ev == SysexEvent2.toInt()) "SysexEvent" else "Unknown"
    }

    /** Return a String representation of a meta-event  */
    private fun MetaName(ev: Int): String {
        return if (ev == MetaEventSequence.toInt()) "MetaEventSequence" else if (ev == MetaEventText.toInt()) "MetaEventText" else if (ev == MetaEventCopyright.toInt()) "MetaEventCopyright" else if (ev == MetaEventSequenceName.toInt()) "MetaEventSequenceName" else if (ev == MetaEventInstrument.toInt()) "MetaEventInstrument" else if (ev == MetaEventLyric.toInt()) "MetaEventLyric" else if (ev == MetaEventMarker.toInt()) "MetaEventMarker" else if (ev == MetaEventEndOfTrack.toInt()) "MetaEventEndOfTrack" else if (ev == MetaEventTempo.toInt()) "MetaEventTempo" else if (ev == MetaEventSMPTEOffset.toInt()) "MetaEventSMPTEOffset" else if (ev == MetaEventTimeSignature.toInt()) "MetaEventTimeSignature" else if (ev == MetaEventKeySignature.toInt()) "MetaEventKeySignature" else "Unknown"
    }
    ...
    private fun parse(rawdata: ByteArray?) {
        ...
        var numer = 0
        var denom = 0
        for (list in allevents!!) {
            for (mevent in list) {
                if (mevent.Metaevent == MetaEventTempo) { // Take average of all tempos
                    tempo += mevent.Tempo.toLong()
                    tempoCount++
                }
        ...
        tempo = if (tempo == 0L) {
            500000 /* 500,000 microseconds = 0.05 sec */
        } else {
            tempo / tempoCount
        }
        if (numer == 0) {
...
//                " start " + starttime + " delta " + mevent.DeltaTime);
    ...
    fun ApplyOptionsToEvents(options: MidiOptions): ArrayList<ArrayList<MidiEvent>> {
        ...
        for (tracknum in newevents.indices) {
        ...
        for (tracknum in newevents.indices) {
            for (mevent in newevents[tracknum]) {
     ...
     */
    fun ApplyOptionsPerChannel(options: MidiOptions): ArrayList<ArrayList<MidiEvent>> { /* Determine which channels to include/exclude.
     ...
     */
    fun GuessMeasureLength(): List<Int> {
    ...
    companion object {
        ...
        const val EventNoteOff = 0x80.toByte()
        const val EventNoteOn = 0x90.toByte()
        const val EventKeyPressure = 0xA0.toByte()
        const val EventControlChange = 0xB0.toByte()
        const val EventProgramChange = 0xC0.toByte()
        const val EventChannelPressure = 0xD0.toByte()
        const val EventPitchBend = 0xE0.toByte()
        const val SysexEvent1 = 0xF0.toByte()
        const val SysexEvent2 = 0xF7.toByte()
        const val MetaEvent = 0xFF.toByte()
        /* The list of Meta Events */
        const val MetaEventSequence = 0x0.toByte()
        const val MetaEventText = 0x1.toByte()
        const val MetaEventCopyright = 0x2.toByte()
        const val MetaEventSequenceName = 0x3.toByte()
        const val MetaEventInstrument = 0x4.toByte()
        const val MetaEventLyric = 0x5.toByte()
        const val MetaEventMarker = 0x6.toByte()
        const val MetaEventEndOfTrack = 0x2F.toByte()
        const val MetaEventTempo = 0x51.toByte()
        const val MetaEventSMPTEOffset = 0x54.toByte()
        const val MetaEventTimeSignature = 0x58.toByte()
        const val MetaEventKeySignature = 0x59.toByte()
     ...
     */
        ...
        fun VarlenToBytes(num: Int, buf: ByteArray, offset: Int): Int {
            val b1 = (num shr 21 and 0x7F).toByte().toInt()
            val b2 = (num shr 14 and 0x7F).toByte().toInt()
            val b3 = (num shr 7 and 0x7F).toByte().toInt()
            val b4 = (num and 0x7F).toByte().toInt()
            return if (b1 > 0) {
                buf[offset] = (b1 or 0x80).toByte()
                buf[offset + 1] = (b2 or 0x80).toByte()
                buf[offset + 2] = (b3 or 0x80).toByte()
                buf[offset + 3] = b4.toByte()
                4
            } else if (b2 > 0) {
                buf[offset] = (b2 or 0x80).toByte()
                buf[offset + 1] = (b3 or 0x80).toByte()
                buf[offset + 2] = b4.toByte()
                3
            } else if (b3 > 0) {
                buf[offset] = (b3 or 0x80).toByte()
                buf[offset + 1] = b4.toByte()
                2
            } else {
                buf[offset] = b4.toByte()
                1
            }
        }
        ...
        private fun WriteEvents(file: FileOutputStream, allevents: ArrayList<ArrayList<MidiEvent>>?,
            ...
            file.write(buf, 0, 2)
            for (list in allevents) { /* Write the MTrk header and track length */
                ...
                for (mevent in list) {
                    ...
                    if (mevent.EventFlag == SysexEvent1 || mevent.EventFlag == SysexEvent2 || mevent.EventFlag == MetaEvent) {
            ...
            for (tracknum in list.indices) {
                ...
                for (mevent in events) {
                    if (mevent.StartTime < pauseTime) {
                        ...
                        if (mevent.EventFlag == EventNoteOn ||
                                mevent.EventFlag == EventNoteOff) { /* Skip NoteOn/NoteOff event */
                        } else if (mevent.EventFlag == EventControlChange) {
                            mevent.DeltaTime = 0
     ...
     */
         ...
         */
        fun CombineToSingleTrack(tracks: ArrayList<MidiTrack>): MidiTrack { /* Add all notes into one track */
            ...
            while (true) {
                ...
                if (lowestnote == null) { /* We've finished the merge */
                    break
                }
                noteindex[lowestTrack]++
                if (prevnote != null && prevnote.startTime == lowestnote.startTime &&
                        prevnote.number == lowestnote.number) { /* Don't add duplicate notes, with the same start time and number */
        ...
        fun CombineToTwoTracks(tracks: ArrayList<MidiTrack>, measurelen: Int): ArrayList<MidiTrack> {
            ...
            for (track in tracks) {
                if (track.lyrics != null) {
                    lyrics.addAll(track.lyrics!!)
                }
            }
            if (lyrics.size > 0) {
                lyrics.sortWith(kotlin.Comparator { o1, o2 -> o1!!.compare(o1, o2!!) })
         ...
         */
        fun RoundStartTimes(tracks: ArrayList<MidiTrack>, millisec: Int, time: TimeSignature?) { /* Get all the starttimes in all tracks, in sorted order */
         ...
         */
        private fun SplitChannels(origtrack: MidiTrack, events: ArrayList<MidiEvent>): ArrayList<MidiTrack> { /* Find the instrument used for each channel */
            ...
            val lyrics = origtrack.lyrics
            if (lyrics != null) {
                for (lyricEvent in lyrics) {
                    for (track in result) {
