...
class ChordSymbol(midinotes: ArrayList<MidiNote>, key: KeySignature,
    ...
    /** The notes to draw  */
    private val accidentalSymbols: Array<AccidentalSymbol?>
    ...
    val stem: Stem?
        get() = if (stem1 == null) {
            ...
            stem1
        } else if (stem1!!.duration.ordinal < stem2!!.duration.ordinal) {
            stem1
        } else {
            stem2
    ...
    override val minWidth: Int
        get() { /* The width needed for the note circles */
            var result = 2 * SheetMusic.NoteHeight + SheetMusic.NoteHeight * 3 / 4
            if (accidentalSymbols.size > 0) {
                result += accidentalSymbols[0]!!.minWidth
                for (i in 1 until accidentalSymbols.size) {
                    val accidental = accidentalSymbols[i]
                    val prev = accidentalSymbols[i - 1]
                    if (accidental!!.note!!.Dist(prev!!.note!!) < 6) {
                        result += accidental.minWidth
                    }
                }
            }
    ...
    override val aboveStaff: Int
        get() { /* Find the topmost note in the chord */
            var topnote = notedata[notedata.size - 1].whitenote!!
            /* The stem.End is the note position where the stem ends.
             * Check if the stem end is higher than the top note.
             */
            if (stem1 != null) topnote = WhiteNote.Max(topnote, stem1!!.end)
            if (stem2 != null) topnote = WhiteNote.Max(topnote, stem2!!.end)
            val dist = topnote.Dist(WhiteNote.Top(clef)) * SheetMusic.NoteHeight / 2
            var result = 0
            if (dist > 0) result = dist
            /* Check if any accidental symbols extend above the staff */
            for (symbol in accidentalSymbols) {
    ...
    override val belowStaff: Int
        get() { /* Find the bottom note in the chord */
            var bottomnote = notedata[0].whitenote!!
            /* The stem.End is the note position where the stem ends.
             * Check if the stem end is lower than the bottom note.
             */
            if (stem1 != null) bottomnote = WhiteNote.Min(bottomnote, stem1!!.end)
            if (stem2 != null) bottomnote = WhiteNote.Min(bottomnote, stem2!!.end)
    ...
    private fun NoteName(notenumber_arg: Int, whitenote: WhiteNote?): String {
        var notenumber = notenumber_arg
        return if (sheetmusic!!.showNoteLetters == MidiOptions.NoteNameLetter) {
            ...
            val notescale = NoteScale.fromNumber(notenumber).value
            ...
            val diff = NoteScale.C.value - mainscale.value
            notenumber += diff
            if (notenumber < 0) {
                notenumber += 12
            }
    ...
    override fun Draw(canvas: Canvas, paint: Paint?, ytop: Int) {
        ...
        val xpos = DrawAccidental(canvas, paint, ytop)
        ...
        /* Draw the stems */
        if (stem1 != null) stem1!!.Draw(canvas, paint, ytop, topstaff)
        if (stem2 != null) stem2!!.Draw(canvas, paint, ytop, topstaff)
     ...
     */
    fun DrawAccidental(canvas: Canvas, paint: Paint?, ytop: Int): Int {
        var xpos = 0
        var prev: AccidentalSymbol? = null
        for (symbol in accidentalSymbols) {
    ...
    fun DrawNotes(canvas: Canvas, paint: Paint?, ytop: Int, topstaff: WhiteNote) {
        ...
        for (note in notedata) { /* Get the x,y position to draw the note */
    ...
    fun DrawNoteLetters(canvas: Canvas, paint: Paint?, ytop: Int, topstaff: WhiteNote) {
        ...
        for (note in notedata) {
            if (!note.leftside) { // There's not enough pixel room to show the letter
                continue
            }
    ...
    companion object {
        ...
        private fun CreateNoteData(midinotes: ArrayList<MidiNote>, key: KeySignature,
            ...
            for (i in 0 until len) {
                ...
                notedata[i]!!.accidental = key.GetAccidental(midi.number, midi.startTime / time.measure)
                if (i > 0 && notedata[i]!!.whitenote!!.Dist(notedata[i - 1]!!.whitenote!!) == 1) { /* This note (notedata[i]) overlaps with the previous note.
            ...
            return notedata as Array<NoteData>
        }

        /** Given the note data (the white keys and accidentals), create
         * the Accidental Symbols and return them.
         */
        private fun CreateAccidentalSymbols(notedata: Array<NoteData>, clef: Clef): Array<AccidentalSymbol?> {
            var count = 0
            for (n in notedata) {
            ...
            val accidentalSymbolArray = arrayOfNulls<AccidentalSymbol>(count)
            var i = 0
            for (n in notedata) {
                if (n.accidental != Accidental.None) {
                    accidentalSymbolArray[i] = AccidentalSymbol(n.accidental, n.whitenote, clef)
                    i++
                }
            }
            return accidentalSymbolArray
        }
        ...
        fun CanCreateBeam(chords: Array<ChordSymbol>, time: TimeSignature, startQuarter: Boolean): Boolean {
            ...
            if (numChords == 6) {
                ...
                if (time.numerator == 6 && time.denominator == 4) { /* first chord must start at 1st or 4th quarter note */
                ...
                if (dur === NoteDuration.Eighth) { /* 8th note chord must start on 1st or 3rd quarter beat */
                    beat = time.quarter * 2
                } else if (dur === NoteDuration.ThirtySecond) { /* 32nd note must start on an 8th beat */
                    beat = time.quarter / 2
                }
                if (chords[0].startTime % beat > time.quarter / 6) {
                    return false
                ...
                /* chord must start on quarter note */
                var beat = time.quarter
                if (time.numerator == 12 && time.denominator == 8) { /* In 12/8 time, chord must start on 3*8th beat */
        ...
        fun LineUpStemEnds(chords: Array<ChordSymbol>) {
            ...
            if (firstStem!!.direction == Stem.Up) { /* Find the highest stem. The beam will either:
            ...
            } else { /* Find the bottommost stem. The beam will either:
            ...
            for (i in 1 until chords.size - 1) {
    ...
    init {
        ...
        accidentalSymbols = CreateAccidentalSymbols(notedata, clef)
        ...
        if (dur1 !== dur2) { /* We have notes with different durations.  So we will need
        ...
        } else { /* All notes have the same duration, so we only need one stem. */
        ...
        if (dur1 === NoteDuration.Whole) stem1 = null
